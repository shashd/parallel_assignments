...
int main (int argc, char * argv[]) {
    ...
    int T; 				//time steps
    int nrequest;  // the number of requested threads
    int nthreads;  // the number of actual threads received
    ...
    if (argc != 4) {
        fprintf(stderr, "Usage: ./exec ArraySize TimeSteps ThreadsNumber\n");
        exit(-1);
    }
    else {
        N = atoi(argv[1]);
        T = atoi(argv[2]);
        nrequest = atoi(argv[3]);
    }
    ...
    for (t = 0 ; t < T; t++) {
        #pragma omp parallel default(shared) private(nbrs) num_threads(nrequest)
        {
            #pragma omp master
            nthreads = omp_get_num_threads(); //no barrier
            #pragma omp for private(j) schedule(static) //i is private by default worksharing
            for (i = 1 ; i < N-1; i++){
                for (j = 1 ; j < N-1; j++) {
    ...
    printf("GameOfLife (%d threads): Size %d Steps %d Time %lf\n", nthreads, N, T, time);
#ifdef OUTPUT
    system(FINALIZE);
#endif
    return 0;
